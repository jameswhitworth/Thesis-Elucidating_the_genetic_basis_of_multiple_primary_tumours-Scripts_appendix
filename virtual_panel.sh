...
#####################################
##Copy number gain called by Canvas##
#####################################

######################################################
##Script to prepare variant table for reading into R##
######################################################

##Subset provided variant table to only include samples of interest
...
rm variant_subset_*.txt
rm SV_canvas_gain_variant_table_*.txt
rm col_1.txt
rm col_2.txt
rm number_variants.txt
rm number_subset_files.txt
rm variant_line_number_ranges_and_header_file_numbers.txt
rm variant_line_number_ranges.txt
rm header_file_numbers.txt

################################################################
##R script to detect variant calls affecting genes of interest##
################################################################
...
##Subset provided variant table to only include samples of interest
fgrep -f samples_without_suffix.txt canvascalls_allsamples_loss.ann.filt.0.01.exons.txt > canvascalls_allsamples_loss.ann.filt.0.01.exons_subset.txt

##Subdivide variant table to reduce processing time
fgrep -f samples_without_suffix.txt canvascalls_allsamples_loss.ann.filt.0.01.exons_subset.txt | wc -l > number_variants.txt 
echo $(( (`cat number_variants.txt`/30) +1 )) > number_subset_files.txt #+1 to round up as bash doesn't use floating point
seq 1 `cat number_subset_files.txt` > header_file_numbers.txt
for i in `cat header_file_numbers.txt`; do
  head -1 canvascalls_allsamples_loss.ann.filt.0.01.exons.txt > variant_subset_${i}.txt
done
seq 2 30 `cat number_variants.txt` > col_1.txt
seq 31 30 `cat number_variants.txt` > col_2.txt
echo `cat number_variants.txt` >> col_2.txt
...
for i in `cat variant_line_number_ranges_and_header_file_numbers.txt`;do
  COL1=`echo ${i} | awk '{ print $1 }'`
  COL2=`echo ${i} | awk '{ print $2 }'`
  sed -n ${COL1}p canvascalls_allsamples_loss.ann.filt.0.01.exons_subset.txt >> variant_subset_${COL2}.txt
done

##Resulting files named variant_subset_1.txt, variant subset_2.txt etc. retained in separate folder for each variant modality  

##R scripts for each variant modality (see below) used to identify structural variant calls fulfilling criteria. Initially used to produce a header line.
Rscript vp_SV_detection_canvas_loss.R 1
head -n 1 SV_canvas_loss_variant_table_1.txt > variants_combined.txt
rm SV_canvas_loss_variant_table_1.txt

##Subsequent execution and output to build variant table with annotation indicating if criteria fulfilled
for i in `cat header_file_numbers.txt`; do
  Rscript vp_SV_detection_canvas_loss.R ${i}
  sed '1d' SV_canvas_loss_variant_table_${i}.txt >> variants_combined.txt
done

##Remove unecessary files
rm variant_subset_*.txt
rm SV_canvas_loss_variant_table_*.txt
rm col_1.txt
rm col_2.txt
rm number_variants.txt
rm number_subset_files.txt
...
##Subset provided variant table to only include samples of interest
fgrep -f samples_without_suffix.txt mantacalls_allsamples_bnd.ann.filt.0.01.txt > mantacalls_allsamples_bnd.ann.filt.0.01.txt_subset.txt

##Subdivide variant table to reduce processing time
fgrep -f samples_without_suffix.txt mantacalls_allsamples_bnd.ann.filt.0.01.txt_subset.txt | wc -l > number_variants.txt 
echo $(( (`cat number_variants.txt`/30) +1 )) > number_subset_files.txt #+1 to round up as bash doesn't use floating point
seq 1 `cat number_subset_files.txt` > header_file_numbers.txt
for i in `cat header_file_numbers.txt`; do
  head -1 mantacalls_allsamples_bnd.ann.filt.0.01.txt > variant_subset_${i}.txt
done
seq 2 30 `cat number_variants.txt` > col_1.txt
seq 31 30 `cat number_variants.txt` > col_2.txt
echo `cat number_variants.txt` >> col_2.txt
...
for i in `cat variant_line_number_ranges_and_header_file_numbers.txt`;do
  COL1=`echo ${i} | awk '{ print $1 }'`
  COL2=`echo ${i} | awk '{ print $2 }'`
  sed -n ${COL1}p mantacalls_allsamples_bnd.ann.filt.0.01.txt_subset.txt >> variant_subset_${COL2}.txt
done

##Resulting files named variant_subset_1.txt, variant subset_2.txt etc. retained in separate folder for each variant modality  

##R scripts for each variant modality (see below) used to identify structural variant calls fulfilling criteria. Initially used to produce a header line.
Rscript vp_SV_detection_manta_bnd.R 1
head -n 1 SV_manta_bnd_variant_table_1.txt > variants_combined.txt
rm SV_manta_bnd_variant_table_1.txt

##Subsequent execution and output to build variant table with annotation indicating if criteria fulfilled
for i in `cat header_file_numbers.txt`; do
  Rscript vp_SV_detection_manta_bnd.R ${i}
  sed '1d' SV_manta_bnd_variant_table_${i}.txt >> variants_combined.txt
done

##Remove unecessary files
rm variant_subset_*.txt
rm SV_manta_bnd_variant_table_*.txt
rm col_1.txt
rm col_2.txt
rm number_variants.txt
rm number_subset_files.txt
...
##Subset provided variant table to only include samples of interest
fgrep -f samples_without_suffix.txt mantacalls_allsamples_del.ann.filt.0.01.exons.txt > mantacalls_allsamples_del.ann.filt.0.01.exons_subset.txt

##Subdivide variant table to reduce processing time
fgrep -f samples_without_suffix.txt mantacalls_allsamples_del.ann.filt.0.01.exons.txt | wc -l > number_variants.txt 
echo $(( (`cat number_variants.txt`/30) +1 )) > number_subset_files.txt #+1 to round up as bash doesn't use floating point
seq 1 `cat number_subset_files.txt` > header_file_numbers.txt
for i in `cat header_file_numbers.txt`; do
  head -1 mantacalls_allsamples_del.ann.filt.0.01.exons.txt > variant_subset_${i}.txt
done
seq 2 30 `cat number_variants.txt` > col_1.txt
seq 31 30 `cat number_variants.txt` > col_2.txt
echo `cat number_variants.txt` >> col_2.txt
...
for i in `cat variant_line_number_ranges_and_header_file_numbers.txt`;do
  COL1=`echo ${i} | awk '{ print $1 }'`
  COL2=`echo ${i} | awk '{ print $2 }'`
  sed -n ${COL1}p mantacalls_allsamples_del.ann.filt.0.01.exons_subset.txt >> variant_subset_${COL2}.txt
done

##Resulting files named variant_subset_1.txt, variant subset_2.txt etc. retained in separate folder for each variant modality  

##R scripts for each variant modality (see below) used to identify structural variant calls fulfilling criteria. Initially used to produce a header line.
Rscript vp_SV_detection_manta_del.R 1
head -n 1 SV_manta_del_variant_table_1.txt > variants_combined.txt
rm SV_manta_del_variant_table_1.txt

##Subsequent execution and output to build variant table with annotation indicating if criteria fulfilled
for i in `cat header_file_numbers.txt`; do
  Rscript vp_SV_detection_manta_del.R ${i}
  sed '1d' SV_manta_del_variant_table_${i}.txt >> variants_combined.txt
done

##Remove unecessary files
rm variant_subset_*.txt
rm SV_manta_del_variant_table_*.txt
rm col_1.txt
rm col_2.txt
rm number_variants.txt
rm number_subset_files.txt
...
##Subset provided variant table to only include samples of interest
fgrep -f samples_without_suffix.txt mantacalls_allsamples_ins.ann.filt.0.01.exons.txt > mantacalls_allsamples_ins.ann.filt.0.01.exons_subset.txt

##Subdivide variant table to reduce processing time
fgrep -f samples_without_suffix.txt mantacalls_allsamples_ins.ann.filt.0.01.exons.txt | wc -l > number_variants.txt 
echo $(( (`cat number_variants.txt`/30) +1 )) > number_subset_files.txt #+1 to round up as bash doesn't use floating point
seq 1 `cat number_subset_files.txt` > header_file_numbers.txt
for i in `cat header_file_numbers.txt`; do
  head -1 mantacalls_allsamples_ins.ann.filt.0.01.exons.txt > variant_subset_${i}.txt
done
seq 2 30 `cat number_variants.txt` > col_1.txt
seq 31 30 `cat number_variants.txt` > col_2.txt
echo `cat number_variants.txt` >> col_2.txt
...
for i in `cat variant_line_number_ranges_and_header_file_numbers.txt`;do
  COL1=`echo ${i} | awk '{ print $1 }'`
  COL2=`echo ${i} | awk '{ print $2 }'`
  sed -n ${COL1}p mantacalls_allsamples_ins.ann.filt.0.01.exons_subset.txt >> variant_subset_${COL2}.txt
done

##Resulting files named variant_subset_1.txt, variant subset_2.txt etc. retained in separate folder for each variant modality  

##R scripts for each variant modality (see below) used to identify structural variant calls fulfilling criteria. Initially used to produce a header line.
Rscript vp_SV_detection_manta_ins.R 1
head -n 1 SV_manta_ins_variant_table_1.txt > variants_combined.txt
rm SV_manta_ins_variant_table_1.txt

##Subsequent execution and output to build variant table with annotation indicating if criteria fulfilled
for i in `cat header_file_numbers.txt`; do
  Rscript vp_SV_detection_manta_ins.R ${i}
  sed '1d' SV_manta_ins_variant_table_${i}.txt >> variants_combined.txt
done

##Remove unecessary files
rm variant_subset_*.txt
rm SV_manta_ins_variant_table_*.txt
rm col_1.txt
rm col_2.txt
rm number_variants.txt
rm number_subset_files.txt
...
##Subset provided variant table to only include samples of interest
fgrep -f samples_without_suffix.txt mantacalls_allsamples_inv.ann.filt.0.01.exons.txt > mantacalls_allsamples_inv.ann.filt.0.01.exons_subset.txt

##Subdivide variant table to reduce processing time
fgrep -f samples_without_suffix.txt mantacalls_allsamples_inv.ann.filt.0.01.exons.txt | wc -l > number_variants.txt 
echo $(( (`cat number_variants.txt`/30) +1 )) > number_subset_files.txt #+1 to round up as bash doesn't use floating point
seq 1 `cat number_subset_files.txt` > header_file_numbers.txt
for i in `cat header_file_numbers.txt`; do
  head -1 mantacalls_allsamples_inv.ann.filt.0.01.exons.txt > variant_subset_${i}.txt
done
seq 2 30 `cat number_variants.txt` > col_1.txt
seq 31 30 `cat number_variants.txt` > col_2.txt
echo `cat number_variants.txt` >> col_2.txt
...
for i in `cat variant_line_number_ranges_and_header_file_numbers.txt`;do
  COL1=`echo ${i} | awk '{ print $1 }'`
  COL2=`echo ${i} | awk '{ print $2 }'`
  sed -n ${COL1}p mantacalls_allsamples_inv.ann.filt.0.01.exons_subset.txt >> variant_subset_${COL2}.txt
done

##Resulting files named variant_subset_1.txt, variant subset_2.txt etc. retained in separate folder for each variant modality  

##R scripts for each variant modality (see below) used to identify structural variant calls fulfilling criteria. Initially used to produce a header line.
Rscript vp_SV_detection_manta_inv.R 1
head -n 1 SV_manta_inv_variant_table_1.txt > variants_combined.txt
rm SV_manta_inv_variant_table_1.txt

##Subsequent execution and output to build variant table with annotation indicating if criteria fulfilled
for i in `cat header_file_numbers.txt`; do
  Rscript vp_SV_detection_manta_inv.R ${i}
  sed '1d' SV_manta_inv_variant_table_${i}.txt >> variants_combined.txt
done

##Remove unecessary files
rm variant_subset_*.txt
rm SV_manta_inv_variant_table_*.txt
rm col_1.txt
rm col_2.txt
rm number_variants.txt
rm number_subset_files.txt
...
##Subset provided variant table to only include samples of interest
fgrep -f samples_without_suffix.txt mantacalls_allsamples_dup.ann.filt.0.01.exons.txt > mantacalls_allsamples_dup.ann.filt.0.01.exons_subset.txt

##Subdivide variant table to reduce processing time
fgrep -f samples_without_suffix.txt mantacalls_allsamples_dup.ann.filt.0.01.exons.txt | wc -l > number_variants.txt 
echo $(( (`cat number_variants.txt`/30) +1 )) > number_subset_files.txt #+1 to round up as bash doesn't use floating point
seq 1 `cat number_subset_files.txt` > header_file_numbers.txt
for i in `cat header_file_numbers.txt`; do
  head -1 mantacalls_allsamples_dup.ann.filt.0.01.exons.txt > variant_subset_${i}.txt
done
seq 2 30 `cat number_variants.txt` > col_1.txt
seq 31 30 `cat number_variants.txt` > col_2.txt
echo `cat number_variants.txt` >> col_2.txt
...
for i in `cat variant_line_number_ranges_and_header_file_numbers.txt`;do
  COL1=`echo ${i} | awk '{ print $1 }'`
  COL2=`echo ${i} | awk '{ print $2 }'`
  sed -n ${COL1}p mantacalls_allsamples_dup.ann.filt.0.01.exons_subset.txt >> variant_subset_${COL2}.txt
done

##Resulting files named variant_subset_1.txt, variant subset_2.txt etc. retained in separate folder for each variant modality  

##R scripts for each variant modality (see below) used to identify structural variant calls fulfilling criteria. Initially used to produce a header line.
Rscript vp_SV_detection_manta_dup.R 1
head -n 1 SV_manta_dup_variant_table_1.txt > variants_combined.txt
rm SV_manta_dup_variant_table_1.txt

##Subsequent execution and output to build variant table with annotation indicating if criteria fulfilled
for i in `cat header_file_numbers.txt`; do
  Rscript vp_SV_detection_manta_dup.R ${i}
  sed '1d' SV_manta_dup_variant_table_${i}.txt >> variants_combined.txt
done

##Remove unecessary files
rm variant_subset_*.txt
rm SV_manta_dup_variant_table_*.txt
rm col_1.txt
rm col_2.txt
rm number_variants.txt
rm number_subset_files.txt
