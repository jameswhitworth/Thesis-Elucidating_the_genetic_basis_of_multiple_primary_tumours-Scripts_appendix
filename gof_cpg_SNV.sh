...
bcftools view -R GOF_exons.bed -e 'FILTER!="PASS"' -o ${i}_filtered.vcf -O v /scratch/WGS10K/data/release/20170614-A/merged-vcf/no_hgmd/${i}.vcf.gz
done
...
bcftools filter -e 'FMT/DP<10 || FMT/GQ<30 || (FMT/AD[1]/(FMT/AD[0] + FMT/AD[1]) <0.3) || (FMT/AD[1]/(FMT/AD[0] + FMT/AD[1] + FMT/AD[2]) <0.3) || (FMT/AD[2]/(FMT/AD[0] + FMT/AD[1] + FMT/AD[2]) <0.3)' -S . -o GOF_MPT_BRIDGE_merged.vcf.gz -O z merged.vcf.gz
...
#./VEP_and_filter_for_candidate_search.sh GOF 1FROM10 Melanoma Lung Bladder GIST Kidney Thyroidmedullary Phaeochromocytoma Paraganglioma Connectivetissuesofttissuesarcoma Haemmyeloid
#./VEP_and_filter_for_candidate_search.sh GOF ALL XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX
...
/home/jww39/ensembl-vep/filter_vep -i VEP_out_${1}_MPT_BRIDGE_ICR_merged.vcf -o VEP_out_filtered_${1}_MPT_BRIDGE_ICR_merged.vcf -filter "Consequence in /home/jww39/candidate_gene_searches/consequence_missense.txt" --only_matched
...
#Rscript candidate_gene_output.R GOF 1FROM10 Melanoma Lung Bladder GIST Kidney Thyroidmedullary Phaeochromocytoma Paraganglioma Connectivetissuesofttissuesarcoma Haemmyeloid
#Rscript candidate_gene_output.R GOF ALL XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX

args = commandArgs(trailingOnly=TRUE)

##Read in necessary files
vcf_to_read <- paste("VEP_out_double_filtered",args[1],"MPT_BRIDGE_ICR_merged.vcf",sep = "_") ##Variant table
...
##Single
if(args[2] == "SINGLE")ting <- rownames(MPT_table[
...
##2 from 2
if(args[2] == "BOTH")ting <- rownames(MPT_table[
...
##1 from 2
if(args[2] == "1FROM2")ting <- rownames(MPT_table[
...
###1 from 3
if(args[2] == "1FROM3")ting <- rownames(MPT_table[
...
###1 from 4
if(args[2] == "1FROM4")ting <- rownames(MPT_table[
...
##1 from 5
if(args[2] == "1FROM5")ting <- rownames(MPT_table[
...
##1 from 6
if(args[2] == "1FROM6")ting <- rownames(MPT_table[
...
###1 from 7
if(args[2] == "1FROM7")ting <- rownames(MPT_table[
...
##1 from 8
if(args[2] == "1FROM8")ting <- rownames(MPT_table[
...
if(args[2] == "1FROM10")ting <- rownames(MPT_table[
  ...
  ,])
##2 from 3
if(args[2] == "2FROM3")ting <- rownames(MPT_table[
...
##2 from 4
if(args[2] == "2FROM4")ting <- rownames(MPT_table[
...
##2 from 5
if(args[2] == "2FROM5")ting <- rownames(MPT_table[
...
##2 from 6
if(args[2] == "2FROM6")ting <- rownames(MPT_table[
...
##2 from 7
if(args[2] == "2FROM7")ting <- rownames(MPT_table[
...
##2 from 8
if(args[2] == "2FROM8")ting <- rownames(MPT_table[
...
if(args[2] != "ALL")indv <- grep("R0*", ting, value = T)
if(args[2] != "ALL")indv <- gsub("_A", "", indv)
if(args[2] != "ALL")indv <- data.frame(indv)
if(args[2] != "ALL")case_list <- indv
if(args[2] == "ALL")case_list <- read.delim("MPT_probands_euro.txt", header = F) 
...
BRIDGE_qvalue_per_gene_hets <- p.adjust(BRIDGE_pvalue_per_gene_hets, method = "fdr", n = 184)
...
ICR_qvalue_per_gene_hets <- p.adjust(ICR_pvalue_per_gene_hets, method = "fdr", n = 184)
...
BRIDGE_qvalue_per_gene_homs <- p.adjust(BRIDGE_pvalue_per_gene_homs, method = "fdr", n = 184)
...
ICR_qvalue_per_gene_homs <- p.adjust(ICR_pvalue_per_gene_homs, method = "fdr", n = 184)
...
BRIDGE_qvalue_per_gene_hets_homs <- p.adjust(BRIDGE_pvalue_per_gene_hets_homs, method = "fdr", n = 184)
...
ICR_qvalue_per_gene_hets_homs <- p.adjust(ICR_pvalue_per_gene_hets_homs, method = "fdr", n = 184)
...
BRIDGE_qvalue_per_gene_hets_homs_agg <- p.adjust(BRIDGE_pvalue_per_gene_hets_homs_agg, method = "fdr", n = 184)
...
ICR_qvalue_per_gene_hets_homs_agg <- p.adjust(ICR_pvalue_per_gene_hets_homs_agg, method = "fdr", n = 184)
...
BRIDGE_qvalue_per_indv_per_gene_hets <- p.adjust(BRIDGE_pvalue_per_indv_per_gene_hets, method = "fdr", n = 184)
...
ICR_qvalue_per_indv_per_gene_hets <- p.adjust(ICR_pvalue_per_indv_per_gene_hets, method = "fdr", n = 184)
...
BRIDGE_qvalue_per_indv_per_gene_homs <- p.adjust(BRIDGE_pvalue_per_indv_per_gene_homs, method = "fdr", n = 184)
...
ICR_qvalue_per_indv_per_gene_homs <- p.adjust(ICR_pvalue_per_indv_per_gene_homs, method = "fdr", n = 184)
...
BRIDGE_qvalue_per_indv_per_gene_hets_homs <- p.adjust(BRIDGE_pvalue_per_indv_per_gene_hets_homs, method = "fdr", n = 184)
...
ICR_qvalue_per_indv_per_gene_hets_homs <- p.adjust(ICR_pvalue_per_indv_per_gene_hets_homs, method = "fdr", n = 184) 
